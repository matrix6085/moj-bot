...
# === KONFIGURACJA ===
TOKEN = os.getenv("TOKEN")  # lub wpisz token bezpośrednio: "TWÓJ_TOKEN"
...
async def save_transcript(channel, ticket_id):
    ...
    if not messages:
        return None
    lines = []
    lines.append(f"=== TRANSCRIPT TICKETU {ticket_id} ===")
    lines.append(f"Kanał: #{channel.name}")
    lines.append(f"Data utworzenia: {datetime.now().strftime('%d.%m.%Y %H:%M:%S')}")
    lines.append("=" * 50)
    for msg in messages:
    ...
    lines.append("=" * 50)
    lines.append(f"Koniec transkryptu - {len(messages)} wiadomości")
    ...
    if config.get("welcome_channel"):
        ...
        if channel:
            ...
            embed = discord.Embed(
                ...
                description=f"Miło Cię widzieć na **{member.guild.name}**! 🎉",
                color=discord.Color.green()
            )
            embed.set_thumbnail(url=member.avatar.url if member.avatar else member.guild.icon.url)
            embed.add_field(name="📅 Data dołączenia", value=f"<t:{int(member.joined_at.timestamp())}:F>", inline=True)
            embed.add_field(name="👥 Liczba użytkowników", value=f"{member_count}", inline=True)
            embed.add_field(name="🔔 Nie zapomnij", value="Przeczytaj regulamin i przywitaj się!", inline=False)
...
async def setticketpanelimage(ctx, url: str = None):
    ...
    save_config(config)
    await ctx.send(f"✅ Ustawiono obrazek panelu" if url else "✅ Usunięto obrazek panelu")
    await ctx.message.delete()
...
async def setclaimrole(ctx, role: discord.Role):
    ...
    save_config(config)
    await ctx.send(f"✅ Ustawiono rolę przejmowania ticketów na {role.mention}")
    await ctx.message.delete()
...
async def setproblememoji(ctx, emoji: str):
    ...
    save_config(config)
    await ctx.send(f"✅ Ustawiono emotkę Problemu: {emoji}")
    await ctx.message.delete()
...
async def setwspolpracaemoji(ctx, emoji: str):
    ...
    save_config(config)
    await ctx.send(f"✅ Ustawiono emotkę Współpracy: {emoji}")
    await ctx.message.delete()
...
async def setkontaktemoji(ctx, emoji: str):
    ...
    save_config(config)
    await ctx.send(f"✅ Ustawiono emotkę Kontaktu: {emoji}")
    await ctx.message.delete()
...
async def showconfig(ctx):
    ...
    embed.add_field(name="📢 Kanał powitalny", value=bot.get_channel(config["welcome_channel"]).mention if config.get("welcome_channel") else "❌ Nie ustawiono", inline=False)
    embed.add_field(name="🎭 Rola dla nowych", value=ctx.guild.get_role(config["welcome_role"]).mention if config.get("welcome_role") else "❌ Nie ustawiono", inline=False)
    embed.add_field(name="🎫 Kategoria ticketów", value=ctx.guild.get_channel(config["ticket_category"]).name if config.get("ticket_category") else "❌ Nie ustawiono", inline=False)
    embed.add_field(name="📋 Kanał panelu", value=bot.get_channel(config["ticket_panel_channel"]).mention if config.get("ticket_panel_channel") else "❌ Nie ustawiono", inline=False)
    ...
    embed.add_field(name="🔧 Rola przejmowania", value=ctx.guild.get_role(config["claim_role"]).mention if config.get("claim_role") else "❌ Nie ustawiono", inline=False)
    embed.add_field(name="😀 Emotki", value=f"Problem: {config.get('problem_emoji', '❌')}\nWspółpraca: {config.get('wspolpraca_emoji', '🤝')}\nKontakt: {config.get('kontakt_emoji', '📞')}", inline=False)
...
async def send_ticket_panel(channel):
    ...
    embed = discord.Embed(
        ...
        description="**Aby skontaktować się z Administracją, wybierz odpowiednią kategorię. Twój bilet automatycznie zostanie dostarczony do właściwych osób.**",
        color=0x5865f2
    )
    ...
    b1 = discord.ui.Button(label="Problem", style=discord.ButtonStyle.secondary, emoji=config.get("problem_emoji", "❌"), custom_id="category_problem")
    b2 = discord.ui.Button(label="Współpraca", style=discord.ButtonStyle.secondary, emoji=config.get("wspolpraca_emoji", "🤝"), custom_id="category_wspolpraca")
    b3 = discord.ui.Button(label="Kontakt z administracją", style=discord.ButtonStyle.secondary, emoji=config.get("kontakt_emoji", "📞"), custom_id="category_kontakt")
    
    async def b1_cb(interaction):
        await show_category_form(interaction, "Problem")
    async def b2_cb(interaction):
        await show_category_form(interaction, "Współpraca")
    async def b3_cb(interaction):
...
async def show_category_form(interaction, category):
    ...
    async def modal_cb(interaction_modal):
...
async def create_ticket(interaction, category, additional_info):
    ...
    try:
        ...
        if config.get("ticket_category"):
            category_obj = interaction.guild.get_channel(config["ticket_category"])
        ...
        embed.set_footer(text=f"{interaction.guild.name} • {current_time}", icon_url=interaction.guild.icon.url)
...
async def show_close_reason(interaction, channel, ticket_id):
    ...
    modal.add_item(discord.ui.TextInput(
        ...
        placeholder="Np. Sprawa rozwiązana, brak odpowiedzi...",
        required=True,
        style=discord.TextStyle.paragraph,
        max_length=500
    ))
...
async def helpme(ctx):
    embed = discord.Embed(title="🤖 Pomoc", color=discord.Color.blue())
    embed.add_field(name="📌 Podstawowe", value="`!ping`, `!hello`, `!helpme`", inline=False)
    if ctx.author.guild_permissions.administrator:
        embed.add_field(name="⚙️ Konfiguracja (admin)", value=
